            ...
            match line {
                Ok(line) => {
                    if line.to_lowercase().contains("error") {
                        let _ = tx_stderr.send(Ok(app::ConsoleText::Stderr(line))); // Adjust as needed
                    } else {
                        let _ = tx_stderr.send(Ok(app::ConsoleText::Stdout(line))); // Handle send error gracefully
                    }
                },
                Err(e) => {
                    let err_msg = e.to_string();
...
pub fn get_ffmpeg(tx: std::sync::mpsc::Sender<Result<std::path::PathBuf, String>>) {
    std::thread::spawn(move || {
        let result = (|| -> Result<std::path::PathBuf, Box<dyn std::error::Error>> {
            let ffmpeg_path = std::env::current_dir()?.join("ffmpeg").join("bin").join("ffmpeg.exe");
            if ffmpeg_path.exists() {
                ...
                return Ok(std::path::PathBuf::from(ffmpeg_path));
            }
        })();
